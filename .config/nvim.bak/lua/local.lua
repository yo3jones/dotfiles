...
local capabilities_cpp = lsp_util.capabilities
...
nvim_lsp.golangci_lint_ls.setup({capabilities = lsp_util.capabilities})
...
function GoFormat()
    ...
    local fmt_job = Job:new({
        ...
        on_exit = function(_, return_val)
            if return_val ~= 0 then errored = true end
        end
    })
    Job:new({
        command = 'golines',
        ...
        writer = fmt_job,
        on_stdout = function(_, d, _) table.insert(data, d) end,
        on_exit = function(_, return_val)
            if return_val ~= 0 then errored = true end
        end
    }):sync()
